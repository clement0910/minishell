...
/*   Updated: 2021/02/10 14:33:59 by csapt            ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	built_in_command(t_global *glb)
{
	if (ft_strcmp(glb->command[0], "echo") == 0)
		...
		return (built_in_export(glb->command, &glb->env, &glb->env_list));
	else if (ft_strcmp(glb->command[0], "unset") == 0)
		return (built_in_unset(glb->command, &glb->env, &glb->env_list));
	return (2);
}
